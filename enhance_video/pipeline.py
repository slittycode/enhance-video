...
from enhance_video.cli import (  # noqa: F401
    QUALITY_PROFILES,
    SUPPORTED_PRESETS,
    SUPPORTED_SCALES,
...
from enhance_video.toolchain import (  # noqa: F401
    Toolchain,
...
def build_pipeline_context(
    ...
    *,
    ...
    return PipelineContext(
        ...
        should_cleanup_workspace=should_cleanup_workspace,
...
def run_plan_only(args: argparse.Namespace) -> int:
    ...
    context = build_pipeline_context(args, create_output_dir=False)

    try:
        info = get_video_info(context.toolchain.ffprobe, context.input_video)
        with contextlib.redirect_stdout(io.StringIO()):
            print("Extracting frames...")
            step_start = time.time()
            with spinner("Extracting frames..."):
                frame_count = ensure_input_frames(
                    context.toolchain.ffmpeg,
                    context.input_video,
                    context.input_frames_dir,
                    args.force,
                )
        ...
        plan_payload: dict[str, object] = {
            ...
            "output_video": str(context.output_video),
        ...
        if args.scene_adaptive:
            ...
            frames = sorted(context.input_frames_dir.glob(INPUT_FRAME_GLOB), key=natural_key)
                ...
                if guardrail_seconds is not None:
                    ...
                    if estimate is None:
                        estimate = estimate_candidate_runtime(
                            ...
                            tile_size=args.tile_size,
                            jobs=args.jobs,
                            model_path=context.toolchain.model_path,
        ...
        if guardrail_seconds is not None:
            save_calibration(context.calibration_path, context.calibration)

        print(json.dumps(plan_payload, indent=2, sort_keys=True))
        return 0
    finally:
        if context.should_cleanup_workspace:
            shutil.rmtree(context.workspace_root, ignore_errors=True)


@_traced
def run_pipeline(args: argparse.Namespace) -> int:
    context = build_pipeline_context(args, create_output_dir=True)
    ...
    output_frames_dir = context.output_frames_dir
    manifest_path = context.manifest_path

    input_identity = build_input_identity(context.input_video)
    manifest_payload = read_workspace_manifest(context.manifest_path)
    ...
    if cached_input_identity and cached_input_identity != input_identity:
        ...
        clear_full_workspace_cache(
            context.input_frames_dir,
            context.output_frames_dir,
    ...
    if args.dry_run:
        print("*** DRY RUN MODE ***")
    print("=" * 60)
    print(f"Input:  {context.input_video}")
    ...
    print(f"Calibration: {context.calibration_path}")
    ...
    try:
        ...
        if context.should_cleanup_workspace:
            ...
            print(f"Workspace kept at: {context.workspace_root}")
