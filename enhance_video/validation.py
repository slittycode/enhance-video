...
def _missing_dependency_message() -> str:
    return (
        "Required validation dependencies are not installed. "
        "Install with: pip install 'enhance-video[validation]'"
    )
...
class VideoValidator:
    ...
    def generate_report(self, input_props: Dict, output_props: Dict) -> Dict[str, Any]:
        ...
        print("  - frame_metrics.csv: Per-frame metrics")
        print("  - summary.json: Summary and worst frames")
        print("  - debug_frames/: Debug images for flagged frames")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not CV2_AVAILABLE:
        print(f"ERROR: {_missing_dependency_message()}")
        return 1
